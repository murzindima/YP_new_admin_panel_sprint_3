...
from config.settings import postgres_settings, elasticsearch_settings, state_settings, app_settings
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            last_modified_person = state_manager.get_state('person_last_modified')
            updated_persons_data, new_last_modified_person = pg_fetcher.fetch_updated_records('person', last_modified_person)
            person_ids = [row[0] for row in updated_persons_data]
            # Fetch updated records for genre
            last_modified_genre = state_manager.get_state('genre_last_modified')
            updated_genres_data, new_last_modified_genre = pg_fetcher.fetch_updated_records('genre', last_modified_genre)
            genre_ids = [row[0] for row in updated_genres_data]
            # Fetch updated records for film work
            last_modified_film_work = state_manager.get_state('film_work_last_modified')
            updated_film_works_data, new_last_modified_film_work = pg_fetcher.fetch_updated_records('film_work', last_modified_film_work)
            film_work_ids = [row[0] for row in updated_film_works_data]
            # Additional related movies by person and genre
            additional_films_by_person = pg_fetcher.fetch_films_by_updated_persons(person_ids)
            additional_films_by_genre = pg_fetcher.fetch_films_by_updated_genres(genre_ids)
            ...
            if not updated_persons_data and not updated_genres_data and not updated_film_works_data:
